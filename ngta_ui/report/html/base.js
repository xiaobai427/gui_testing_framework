...
function toggleTestCaseResult(selector)
{
    ...
    var a = parent.querySelector("#operation-toggle-testsuite-results");
...
function toggleTestSuiteResults(tsRecordId, toggleTarget){
    ...
    var a = tsRecord.querySelector(".operation-toggle-testsuite-results");
...
function onDisplayResultBlockCheck(target, selector) {
...
function getTimelineAndDurationDataGroupByStatus(tcRecordSelector) {
     ...
     for(var i=0; i < tcRecordElements.length; i++) {
        ...
        var tcRecord = {
            ...
            started_at: tcRecordElement.querySelector("td.column-start-time").textContent.trim(),
            stopped_at: tcRecordElement.querySelector("td.column-finish-time").textContent.trim(),
            duration: parseFloat(tcRecordElement.querySelector("td.column-duration").textContent.trim().replace('s', '')),
            testbench_name: tcRecordElement.querySelector("td.column-bench-name").textContent.trim(),
        };

        middleTime = Date.parse(tcRecord.started_at) + (tcRecord.duration * 1000 / 2)
                ...
                tooltip: {
                    ...
                    formatter: function (obj) {
                        ...
                        return '<div style="border-bottom: 1px solid rgba(255,255,255,.3); font-size: 18px;padding-bottom: 7px;margin-bottom: 7px">'
                            ...
                            + 'Start Time: ' + record.started_at + '<br>'
                            + 'Finish Time: ' + record.stopped_at + '<br>'
